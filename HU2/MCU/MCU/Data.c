...
ISR(CANIT_vect)
{
	...
	if(ReceiveAddress == MASTERID)
	{
		...
		if(ui_current_screen == SCREEN_TEST)
		{
			...
			if(length == 2)			test_value = (ReceiveData[0] << 8) + (ReceiveData[1]);
			else if(length == 3)	test_value = (ReceiveData[0] << 16) + (ReceiveData[1] << 8) + (ReceiveData[2]);
			else if(length >= 4)	test_value = (ReceiveData[0] << 24) + (ReceiveData[1] << 16) + (ReceiveData[2] << 8) + (ReceiveData[3]);
